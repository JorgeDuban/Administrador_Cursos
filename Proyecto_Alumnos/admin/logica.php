<?php 
    ...
    {
        $stm = 
            $conn->prepare("SELECT `login` FROM `usuario` WHERE `login` = :Login and `password` = :Password");
...
class sentencias_sql {
    ...
    function actualizar_codigo($conn, $Codigo_aleatorio, $correo){
        $nuevo_codigo= md5($Codigo_aleatorio);
                    $stm = 
            $conn->prepare("UPDATE usuario SET password=:codigo, codigo_recuperacion=:codigo WHERE `email` = :correo");
    ...
    function buscar_usuario($conn, $correo){
                    $stm = 
            $conn->prepare("SELECT login, nombre From usuario Where email =:correo");
    ...
    function actualizar_password($conn, $password, $codigo_recuperacion){
        $stm = 
        $conn->prepare("UPDATE usuario SET password=:password WHERE `codigo_recuperacion` = :codigo_recuperacion");
